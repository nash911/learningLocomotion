  ...
  switch(robot_type)
  {
    ...
    default:
    {
      std::cerr << "Morphomotion Error: Robot class." << std::endl
                ...
                << "Unknown RobotType: " << robot_type << std::endl;
      exit(1);
    }
  }
}
...
void Robot::set_evaluation_method(const std::string& new_evaluation_method)
{
  ...
  else if(new_evaluation_method == "Distance_Forward")
  {
    evaluation_method = Distance_Forward;
  }
  else
  {
    std::cerr << "Morphomotion Error: Robot class." << std::endl
...
double Robot::get_distance_travelled(void)
{
  ...
  {
    return(this->calculate_total_distance_travelled_euclidean());
  }
  else if(evaluation_method == Euclidean_Distance_Cumulative)
  {
    return(distance_travelled);
  }
  else if(evaluation_method == Distance_Forward)
  {
    return(this->calculate_distance_travelled_forward());
  }
  else
  {
    std::cerr << "Morphomotion Error: Robot class." << std::endl
...
unsigned long Robot::get_previous_velocity_time(void)
{
    return(previous_velocity_time);
}
