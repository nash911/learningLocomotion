...
VisualTracker::VisualTracker(int w, int h)
{
    printf("VisualTracker ctor(%ix%i)\n", w, h);
    /***********************OpenCV Debugger****************************
   cv::Mat image;
   ...
   ***********************OpenCV Debugger****************************/
    ...
    printf("camera res %gx%g\n", camera.get(CV_CAP_PROP_FRAME_WIDTH),
           camera.get(CV_CAP_PROP_FRAME_HEIGHT));
    camera.set(CV_CAP_PROP_FRAME_WIDTH, w);
    camera.set(CV_CAP_PROP_FRAME_HEIGHT, h);
    printf("camera res %gx%g\n", camera.get(CV_CAP_PROP_FRAME_WIDTH),
           camera.get(CV_CAP_PROP_FRAME_HEIGHT));
    //if (camera.set(CV_CAP_PROP_FRAME_WIDTH, w) || camera.set(CV_CAP_PROP_FRAME_HEIGHT, h)) {
    //    printf("Could not set camera res %ix%i\n", w, h);
    //    if (camera.set(CV_CAP_PROP_FRAME_WIDTH, DEFAULT_WIDTH)
    //            || camera.set(CV_CAP_PROP_FRAME_HEIGHT, DEFAULT_HEIGHT)) {
    //        printf("Could not set camera default res %ix%i\n", DEFAULT_WIDTH, DEFAULT_HEIGHT);
    //        exit(-1);
    //    }
    //}
} // end ctor
