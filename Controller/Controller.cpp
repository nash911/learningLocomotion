...
void Controller::set_robot_primary(Robot* pointer_robot_primary)
{
...
void Controller::load_extendedtrianglesquare_control_parameters(void)
{
  unsigned int independent_parameter_index = 0;
  ...
  for(unsigned int module=0; module<number_of_modules; module++)
  {
    set_D0(mlp->get_independent_parameter(independent_parameter_index), module, independent_parameter_index);
    independent_parameter_index++;
  }
  ...
  for(unsigned int module=0; module<number_of_modules; module++)
  {
    set_D1(mlp->get_independent_parameter(independent_parameter_index), module, independent_parameter_index);
    independent_parameter_index++;
  }
  ...
  for(unsigned int module=0; module<number_of_modules; module++)
  {
    set_sk0(mlp->get_independent_parameter(independent_parameter_index), module, independent_parameter_index);
    independent_parameter_index++;
  }
  ...
  for(unsigned int module=0; module<number_of_modules; module++)
  {
    set_sk1(mlp->get_independent_parameter(independent_parameter_index), module, independent_parameter_index);
    independent_parameter_index++;
  }
  ...
  //-- Load sp (Stabilization Period)
  for(unsigned int module=0; module<number_of_modules; module++)
  {
    set_sp(mlp->get_independent_parameter(independent_parameter_index), module, independent_parameter_index);
    independent_parameter_index++;
  }
  ...
  //-- HOAP Related
  if(robot_primary->get_robot_type() == "Hoap3")
  {
      //--Fixing Sholder Roll joints with an Amplitude of 0º and offset of 10º.
      A[7][0] = A[17][0] = 0;
      A[7][1] = A[17][1] = 0;

      offset[7] = offset[17] = -10;

      //--Fixing Sholder Pitch joint offset to 90º.
      offset[6] = 90;

      //-- Fixing Offset of Hip roll and Ankle roll joints to 0º.
      offset[1] = offset[5] = 0;

      //-- Symmetry between Hip roll and Ankle roll joints.
      //-- Only the signs of Amplitude of Hip roll and Ankle roll joints are made to be the same.
      A[5][0] = A[5][0] * (sgn(A[1][0]) * sgn(A[5][0]));
      A[5][1] = A[5][1] * (sgn(A[1][1]) * sgn(A[5][1]));
      ...
      //-- Symmetry between Right and Left legs
      for(unsigned int module=1; module<=6; module++)
      {
          A[module+10][0] = A[module][0];
          A[module+10][1] = A[module][1];

          s[module+10][0] = s[module][0];
          s[module+10][1] = s[module][1];

          a[module+10][0] = a[module][0];
          a[module+10][1] = a[module][1];

          D[module+10][0] = D[module][0];
          D[module+10][1] = D[module][1];

          sk[module+10][0] = sk[module][0];
          sk[module+10][1] = sk[module][1];

          offset[module+10] = offset[module];
          sp[module+10] = sp[module];

          phase[module+10] = phase[module] + M_PI;
          //recalculate_phase(phase[module+10]);
      }
      ...
      /*for(unsigned int module=1; module<=6; module++)
      {
          std::cout << "Module[" << module << "]: " << phase[module] << "     Module[" << module+10 << "]: " << phase[module+10] << std::endl;
      }*/
  }
...
void Controller::set_D0(const double new_D0, const unsigned int module, const unsigned int independent_parameter_index)
{
  if(new_D0 >= mlp->get_independent_parameter_minimum(independent_parameter_index) && new_D0 <= mlp->get_independent_parameter_maximum(independent_parameter_index))
  {
    D[module][0] = new_D0;
  }
  else
  {
    std::cerr << "Morphomotion Error: Controller class." << std::endl
              << "void set_D0(const double, const unsigned int, const unsigned int) method." << std::endl
              << "D0 should be between Min: " << mlp->get_independent_parameter_minimum(independent_parameter_index) << " and Max: " << mlp->get_independent_parameter_maximum(independent_parameter_index) << std::endl
              << "D0[" << independent_parameter_index%number_of_modules << "]: " << new_D0 << std::endl;
    exit(1);
  }
}


void Controller::set_D1(const double new_D1, const unsigned int module, const unsigned int independent_parameter_index)
{
  if(new_D1 >= mlp->get_independent_parameter_minimum(independent_parameter_index) && new_D1 <= mlp->get_independent_parameter_maximum(independent_parameter_index))
  {
    D[module][1] = new_D1;
  }
  else
  {
    std::cerr << "Morphomotion Error: Controller class." << std::endl
              << "void set_D1(const double, const unsigned int, const unsigned int) method." << std::endl
              << "D1 should be between Min: " << mlp->get_independent_parameter_minimum(independent_parameter_index) << " and Max: " << mlp->get_independent_parameter_maximum(independent_parameter_index) << std::endl
              << "D1[" << independent_parameter_index%number_of_modules << "]: " << new_D1 << std::endl;
    exit(1);
  }
}


void Controller::set_sk0(const double new_sk0, const unsigned int module, const unsigned int independent_parameter_index)
{
  if(new_sk0 >= mlp->get_independent_parameter_minimum(independent_parameter_index) && new_sk0 <= mlp->get_independent_parameter_maximum(independent_parameter_index))
  {
    sk[module][0] = new_sk0;
  }
  else
  {
    std::cerr << "Morphomotion Error: Controller class." << std::endl
              << "void set_sk0(const double, const unsigned int, const unsigned int) method." << std::endl
              << "sk0 should be between Min: " << mlp->get_independent_parameter_minimum(independent_parameter_index) << " and Max: " << mlp->get_independent_parameter_maximum(independent_parameter_index) << std::endl
              << "sk0[" << independent_parameter_index%number_of_modules << "]: " << new_sk0 << std::endl;
    exit(1);
  }
}


void Controller::set_sk1(const double new_sk1, const unsigned int module, const unsigned int independent_parameter_index)
{
  if(new_sk1 >= mlp->get_independent_parameter_minimum(independent_parameter_index) && new_sk1 <= mlp->get_independent_parameter_maximum(independent_parameter_index))
  {
    sk[module][1] = new_sk1;
  }
  else
  {
    std::cerr << "Morphomotion Error: Controller class." << std::endl
              << "void set_sk1(const double, const unsigned int, const unsigned int) method." << std::endl
              << "sk1 should be between Min: " << mlp->get_independent_parameter_minimum(independent_parameter_index) << " and Max: " << mlp->get_independent_parameter_maximum(independent_parameter_index) << std::endl
              << "sk1[" << independent_parameter_index%number_of_modules << "]: " << new_sk1 << std::endl;
    exit(1);
  }
}
...
void Controller::set_sp(const double new_sp, const unsigned int module, const unsigned int independent_parameter_index)
{
  if(new_sp >= mlp->get_independent_parameter_minimum(independent_parameter_index) && new_sp <= mlp->get_independent_parameter_maximum(independent_parameter_index))
  {
    sp[module] = new_sp;
  }
  else
  {
    std::cerr << "Morphomotion Error: Controller class." << std::endl
              << "void set_sp(const double, const unsigned int, const unsigned int) method." << std::endl
              << "sp must be between: " << mlp->get_independent_parameter_minimum(independent_parameter_index) << " and: " << mlp->get_independent_parameter_maximum(independent_parameter_index) << std::endl
              << "sp[" << independent_parameter_index%number_of_modules << "]: " << new_sp << std::endl;
    exit(1);
  }
}
...
void Controller::set_controller_type(const std::string& new_controller_type)
{
  ...
  else if(new_controller_type == "ExtendedTriangleSquare_Controller")
  {
    controller_type = ExtendedTriangleSquare_Controller;
  }
  else
  {
    std::cerr << "Morphomotion Error: Controller class." << std::endl
...
std::string Controller::get_controller_type(void)
{
  switch(controller_type)
  {
    ...
    case ExtendedTriangleSquare_Controller:
    {
      return("ExtendedTriangleSquare_Controller");
    }
    default:
    {
      std::cerr << "Morphomotion Error: Controller class." << std::endl
...
bool Controller::validate_amplitude_plus_offset(const double amplitude, const double offset)
{
  ...
  {
    std::cout << std::endl << "amplitude: " << amplitude << "   offset: "  << offset << std::endl;
    return true;
  }
...
bool Controller::validate_amplitude_plus_offset(const double amplitude, const double offset, const unsigned int module)
{
  std::cout << std::endl << "Module: " << module << "  Amplitude: " << amplitude << "   Offset: "  << offset << std::endl;
...
void Controller::recalculate_phase(double& phase)
{
    if(phase > M_PI)
    {
